...
import { User, Phone, MapPin, FileText, UserCheck, ChevronLeft, CheckCircle2 } from "lucide-react";
  ...
  if (success) {
    return (
      <div className="card-lg p-12 text-center mt-8">
        ...
        <h3 className="text-xl font-extrabold text-slate-900 mb-1">विवरण परिमार्जन भयो!</h3>
        <p className="text-slate-500 font-medium">Customer updated successfully</p>
      </div>
    );
  }

  if (fetching) {
    return <div className="p-8 text-center text-slate-400 text-lg font-medium animate-pulse">Loading...</div>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/customers/${id}`} className="w-10 h-10 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-600 hover:bg-slate-50 active:scale-95 transition-all">
          <ChevronLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">ग्राहक सम्पादन</h1>
          <p className="text-sm text-slate-500 font-medium">Edit Customer Details</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card-lg p-6 space-y-5">
        ...
        >
          {loading ? (
            ...
            <>
              <UserCheck size={18} />
              <span>परिमार्जन गर्नुहोस् (Update)</span>
            </>
          )}
        </button>
      </form>
